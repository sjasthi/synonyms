...
echo implode($usedWords, ",");
echo "<br>";
echo implode($final_clue_words, ",");
    foreach($usedWords as $myWord){

        $clueIDQuery = "SELECT ClueID FROM synonyms WHERE SynonymWord Like '$myWord'"; 
        $stmt = $db->prepare($clueIDQuery);
        $stmt->execute();
        $stmt->store_result();
        $stmt->bind_result($clueID);
        ...
        $stmt->fetch();
        echo " master word is: ", $masterWord. "<br>";
       ...
       // if ($masterWord != $tempselectedWord){
                ...
                // will all have the same master word.
                    ...
                    $tempSelectedWordchars = getWordChars($myWord);
