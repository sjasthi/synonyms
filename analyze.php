...
function analyze_essay($essay, $language) {
	...
	$analysis .= "|word number: ".$word_num .", word = ". $current->getWord() .", length = ". $current->getLength() .", frequency: ". $freq;
	// words|letters|lines|strength|weight|avg strength|avg weight
	echo 'total words: '.$tot_words ."| total letters:". $tot_letters ."| total lines: ". $tot_lines;
	echo "|" . $tot_str ."|". $tot_wt;
	echo "|". $tot_str / $tot_words . "|" . $tot_wt / $tot_words;
	echo "<br>analysis = ".$analysis;
}//end analyze_essay
